...
class Agent:
    ...
    def __init__(self, session, num_actions, width, height, path, writer=None):
        ...
        self.main_dqn = DeepQNetwork(session, height, width, num_actions, "main", writer)
        self.target_dqn = DeepQNetwork(session, height, width, num_actions, "target", None)
    ...
    def replay(self, cnt):
        ...
        self.main_dqn.perform_training(states, actions, target, cnt)

    def explore_less(self):
        self.explore_prob = max(self.explore_min, self.explore_prob * self.explore_decay)

    def update_target_network(self):
        self.target_dqn.copy_variables(self.main_dqn)

    def save(self, cnt):
        save_path = self.saver.save(self.session, self.path_checkpoints + "model.checkpoint", global_step=cnt)
