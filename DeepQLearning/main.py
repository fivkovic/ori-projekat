...
def play(agent, env, preprocessor):
    ...
    name = FLAGS.logdir + "model.checkpoint"
    ...
    while True:
        ...
        frame = preprocessor.process_raw_image(frame)
        #Image.fromarray(frame).show()
        ...
        while not crashed:
            ...
            if env.actions[action] == "FORTH":
                pass
            else:
                print("action: {}".format(env.actions[action]))

            next_frame = preprocessor.process_raw_image(next_frame)
...
def main(_):
    ...
    agent = Agent(session, num_actions, width, height, FLAGS.logdir, writer)
    preprocessor = Utility(width, height)
